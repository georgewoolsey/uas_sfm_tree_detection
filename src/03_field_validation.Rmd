...
Pick a DBH to use for the validation. For this project, the UAS point cloud processing via the [`cloud2trees` package](https://github.com/georgewoolsey/cloud2trees) intitially utilized the linear model to estimate missing DBH values using training values extracted from the point cloud using the `TreeLS` package. For the research article we used the regional DBH model.
